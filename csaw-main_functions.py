...
class projector(nn.Module):
    ...
    def forward(self,im_features):
        ...
        # print(f'im_prompt:{im_prompt.shape}')
        # exit()
        return im_prompt
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.CSAWNet.N_CTX
        ctx_init = cfg.TRAINER.CSAWNet.CTX_INIT
...
def jigsaw(batch_image):
    perms = np.load("/home/biplab/Avigyan/models/perms.npy")
    resize = T.Resize(256)
    ...
    #reshuffled_image_batch = torch.stack(reshuffled_image_list)
...
class UpsampleNetwork(nn.Module):
    def __init__(self):
        ...
        # self.conv5 = nn.ConvTranspose2d(28, 3, kernel_size=9, stride=5, padding=1)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label):
        ...
        # masks = mask_images(jigsaw_image).to(device_cuda) 
        # masked_images = jigsaw_image * masks

        # print(f"masks : {masks}")
        # print(f"masks shape : {masks.shape}")
        # print(f"jigsaw_image : {jigsaw_image}")
        # print(f"masked_images : {masked_images}")
        # exit()
        ...
        for pts_i, jigpts_i, imf_i in zip(image_prompts, jigsaw_prompts, jigsaw_image_features):
            ...
            print(image_text_features.shape)
            ...
            # print(f'pts_i : {pts_i.dtype}')
            # print(f'logit_scale : {logit_scale.dtype}')
            print(imf_i.shape)
            l_i = logit_scale * imf_i @ average_text_features.t()
            print(l_i.shape)
            jigsaw_logits.append(l_i)
            print(jigsaw_logits.shape)
            exit()
        ...
        #print(f'output_images : {output_images.shape}')
        target_size = (224, 224)
        ...
        # print(f'original_images : {original_images.dtype}')
        # print(f'reconstructed_images : {reconstructed_images.dtype}')
        # exit()
...
class CSAWLoss(_Loss):
    ...
    def forward(self, jigsaw_logits, image_features, jigsaw_image_features, reconstructed_images, jigsaw_ctx_shifted, original_images, label):
        ...
        #print(f'ce_loss : {ce_loss.dtype}')
        # print(f'jigsaw_logits : {jigsaw_logits}')
        # print(f'jigsaw_logits : {jigsaw_logits.shape}')
        # exit()
        ...
        reconstruction_loss = mse(original_images, reconstructed_images) 
        # print(f'mse_loss : {mse_loss.dtype}')
        exit()
        
        ssl_loss = self.ssl(image_features, jigsaw_image_features)
        #print(f'ssl_loss : {ssl_loss}')

        # probs = torch.nn.functional.softmax(jigsaw_logits, dim=1)
        
        # # probs = F.softmax(jigsaw_logits, dim=1)
        
        # # Step 2: Compute entropy
        # # entropy = - torch.sum(-probs * torch.log2(probs + 1e-10))

        # entropyloss = self.entropy(probs)

        # probs = F.softmax(jigsaw_logits, dim=1)
        # # Calculate the negative log probabilities
        # neg_log_probs = -torch.log(probs)
        # # Calculate the entropy loss by taking the mean across the batch and sequence dimensions
        # entropyloss = torch.mean(torch.sum(probs * neg_log_probs, dim=1))
        # print(f'entropyloss : {entropyloss}')

        # total_loss = ce_loss + 0.1*torch.log10(-sim_loss) + 0.001*ssl_loss   # lambda = 0.1 : weight
        # total_loss = ce_loss + 0.01*(-sim_loss) + 0.001*ssl_loss + 0.1*entropyloss   # lambda = 0.1 : weight

        #total_loss = ce_loss + 0.01*(-sim_loss) + 0.001*ssl_loss
        total_loss = ce_loss + (0.6*((0.002*ssl_loss) + reconstruction_loss)) + ((1-0.6)*0.02*(-crp_loss))
        #print(f'total_loss : {total_loss}')
        return total_loss


@TRAINER_REGISTRY.register()
class CSAWNet(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.CSAWNet.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.CSAWNet.PREC == "fp32" or cfg.TRAINER.CSAWNet.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.CSAWNet.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        # n_ctx1 = torch.tensor(n_ctx, dtype=torch.float16)

        model = self.model
        optim = self.optim
        scaler = self.scaler
        
        prec = self.cfg.TRAINER.CSAWNet.PREC
        if prec == "amp":
            with autocast():
                ...
                # with torch.no_grad():
                #     model_output = self.model(image, label)
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            # output,  label, aug_logits, aug_label = self.model_inference(input, label)
